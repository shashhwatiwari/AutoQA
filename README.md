...
**Negative and investigative tests** probe the boundary between expected and undefined behaviour. Login attempts use whitespace-only strings, 500-character inputs, special characters, Unicode, XSS payloads, and emoji to determine what the application accepts or rejects. Checkout form fields receive alphabetic postal codes, SQL injection strings, and newline characters. Four tests are marked `xfail` — they document known application gaps without breaking CI. Three track SauceDemo's inconsistent empty-cart checkout behaviour (which has toggled between blocking and allowing checkout without items across deployments). One tracks that direct URL access to `/checkout-step-one.html` with a valid session renders the checkout form regardless of cart state. `xfail` turns these unstable behaviours into tracked artefacts rather than silent holes.
